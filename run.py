...
def api_chat():
    ...
    if any(w in mensaje for w in ['pedido', 'orden', 'compra']):
        ...
        resp = f"Hay {len(pedidos)} pedidos en total. {pendientes} pendientes y {proceso} en proceso."
        ...
        resp = f"⚠️ Stock crítico en: {', '.join(bajos)}. Total disponible: {total}." if bajos else f"Inventario bien. {total} unidades disponibles."
    elif any(w in mensaje for w in ['venta', 'ingreso', 'dinero', 'total']):
        total = sum(p['total'] for p in pedidos if p['estado'] == 'Entregado')
        resp  = f"Ventas confirmadas: ${total:,}."
    elif any(w in mensaje for w in ['cliente']):
        resp = f"Hay {len([c for c in clientes if c.get('activo', True)])} clientes activos."
    elif any(w in mensaje for w in ['hola', 'ayuda']):
        resp = "👋 Hola! Puedo ayudarte con pedidos, inventario, ventas y predicciones."
    else:
        resp = "Puedo ayudarte con pedidos, inventario, ventas y predicciones. ¿Qué necesitas?"
